...
- `/health` 是纯 liveness，不创建 Session、不执行 SQL；`/ready` 仅执行最低限度的异步数据库连通性检查。两者不使用业务响应包装，也不执行迁移、建表或业务查询。
- `main.py` 只做应用装配；业务规则放在资源附近，不放入 `core/`。只有真实且稳定的复用需求出现时，才提取 service/use-case 或 repository。
- 新能力改变公开 HTTP 契约时，先明确路径、字段、状态码、错误语义与权限要求；文件、流式响应、Webhook、幂等键、限流或 WebSocket 等能力按真实需求单独设计，不套用普通 JSON 响应约定或预建实现。
- 修改后补充成功路径和明确失败分支的最小测试，验证 OpenAPI Schema 与 docs，同步受影响的 README 契约。路径及开关配置以 README 为准。
