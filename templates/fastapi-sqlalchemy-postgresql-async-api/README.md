# FastAPI + SQLAlchemy + PostgreSQL Async API 模板

业务无关的异步 PostgreSQL API 基础模板，提供 FastAPI、SQLAlchemy 2.x AsyncIO、asyncpg、Alembic、Pydantic v2、pytest 和 Ruff 的最小可运行配置，以及响应、错误、探针与请求日志约定。

模板只支持 PostgreSQL，只包含异步数据库基础设施，不包含业务模型、认证、AI SDK、流式端点、pgvector、任务队列、缓存、WebSocket、Docker 或 CI。
...
编辑 `.env`，填写一个**已经存在**的 PostgreSQL database 及其可登录用户。首次使用模板、且 `DB_NAME` 指向的新 database 尚不存在时，需要先创建这个数据库：
...
模板通过 SQLAlchemy `URL.create()` 构造 `postgresql+asyncpg` URL。不要把真实凭据提交到仓库。
