# FastAPI + SQLAlchemy + PostgreSQL Async API 项目

业务无关的异步 PostgreSQL API 基础项目，提供 FastAPI、SQLAlchemy 2.x AsyncIO、asyncpg、Alembic、Pydantic v2、pytest 和 Ruff 的最小可运行配置，以及响应、错误、探针与请求日志约定。

项目只支持 PostgreSQL，只包含异步数据库基础设施，不包含业务模型、认证、AI SDK、流式端点、pgvector、任务队列、缓存、WebSocket、Docker 或 CI。
...
HTTPException 的 `detail` 仅能填写已经审查、可公开的字符串；dict、list、异常对象或原始第三方响应不会被回显。当前项目只承载已实现的业务无关基础能力；公开 API、数据模型、认证/权限、迁移或部署边界应由实际项目需求明确，不能以示例业务或空架构层代替决策。
...
- `uv run --locked uvicorn app.main:app --reload --no-access-log`：在项目虚拟环境启动开发服务，并在源码修改时自动重载。
...
APP_NAME=FastAPI SQLAlchemy PostgreSQL Async API
APP_VERSION=1.0.0
API_PREFIX=/api/v1
...
项目通过 SQLAlchemy `URL.create()` 构造 `postgresql+asyncpg` URL。不要把真实凭据提交到仓库。

业务端点通过 `get_db` 获得请求级 AsyncSession；它只在请求结束后关闭 Session，不会自动提交。端点或用例必须显式决定 `await session.commit()` 与 `await session.rollback()`，且不得在并发 task 之间共享同一个 AsyncSession。Session 使用 `expire_on_commit=False`，依赖数据库默认值或 trigger 生成值时应显式 `await session.refresh(instance)`。
...
Schema 变更统一通过 Alembic 管理，不在应用启动时调用 `create_all()` 或自动执行迁移。Alembic online 连接使用 async engine，并通过 `run_sync` 执行 migration context；revision 中的 `upgrade()` 和 `downgrade()` 仍保持同步函数。
...
```bash
uv run --locked uvicorn app.main:app --host 0.0.0.0 --port 8000 --no-access-log
```
