# FastAPI + SQLAlchemy + PostgreSQL 模板协作规则
...
- 模板只支持 PostgreSQL，保持同步 SQLAlchemy 2.x 和 Psycopg 3；不得加入其他数据库兼容分支或仅因偏好改为异步数据库栈。
...
- 业务接口通过 `get_db` 取得请求级 Session，并自行决定 `commit()` 或 `rollback()`；不得把自动提交加入依赖。
