...
- Python 3.12
- uv
- 执行迁移或数据库集成测试时，需要可访问的 MySQL 8.x
...
- `uv run --locked alembic upgrade head`：将**已有** database 升级到最新 migration；它不会创建 MySQL database，可能创建 `alembic_version` 表。
...
APP_NAME=FastAPI SQLAlchemy MySQL API
APP_VERSION=1.0.0
API_PREFIX=/api/v1
...
默认测试会验证配置、响应/错误契约、探针、请求 ID、`get_db` 生命周期和 `/health`；如果没有显式提供全部 `DB_*`，真实 MySQL 集成测试会明确跳过。使用专用、可丢弃的测试数据库执行完整测试：
...
使用同一组 `DB_*` 单独执行 `uv run --locked alembic upgrade head`，可验证 Alembic 与 MySQL 的连接。迁移可能创建 `alembic_version` 表，不要指向已有业务数据库。
