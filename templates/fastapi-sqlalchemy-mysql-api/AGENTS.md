...
- 保持 MySQL-only、同步 SQLAlchemy 2.x 与 PyMySQL；不因偏好改为异步或增加数据库兼容分支。
...
- `/health` 不访问数据库；`/ready` 只检查数据库连通性，不迁移、建表或执行业务查询。
- Schema 只通过 Alembic 变更并人工审查 revision；禁止 `create_all()` 和启动自动迁移。`get_db` 不自动提交，业务显式决定事务提交与回滚。
...
- 完成前执行 README 的安装、静态检查、测试和构建命令，按实际变更追加迁移、OpenAPI 或外部依赖验证；MySQL 集成测试未执行或跳过时明确报告数据库行为未验证。
