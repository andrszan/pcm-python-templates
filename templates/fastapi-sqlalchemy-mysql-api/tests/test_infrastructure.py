...
has_mysql_config = all(name in os.environ for name in DB_ENV_NAMES)
...
def test_database_url_and_settings() -> None:
    ...
    assert settings.app_name == "FastAPI SQLAlchemy MySQL API"
...
def test_versioned_api_and_documentation() -> None:
    ...
    assert app.title == "FastAPI SQLAlchemy MySQL API"
...
def test_mysql_connection() -> None:
    ...
    with engine.connect() as connection:
        ...
        assert connection.scalar(text("SELECT @@character_set_client")) == "utf8mb4"
